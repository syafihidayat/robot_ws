#include <rclcpp/rclcpp.hpp>
#include <geometry_msgs/msg/twist.hpp>
#include <nav_msgs/msg/odometry.hpp>
#include <geometry_msgs/msg/point.hpp>
#include <std_msgs/msg/bool.hpp>
#include <visualization_msgs/msg/marker.hpp>
#include <cmath>

class Movement : public rclcpp::Node
{
public:
  Movement() : Node("Movement_Point")
  {
    cmd_pub = this->create_publisher<geometry_msgs::msg::Twist>("/omni_cont/cmd_vel", 10);

    odom_sub = this->create_subscription<nav_msgs::msg::Odometry>("/odom", 10,
      std::bind(&Movement::odom_callback, this, std::placeholders::_1));

    pose_sub = this->create_subscription<geometry_msgs::msg::Point>("/pose", 10,
      std::bind(&Movement::pose_callback, this, std::placeholders::_1));

    reached_pub = this->create_publisher<std_msgs::msg::Bool>("/target_reached", 10);

    visual_pub = this->create_publisher<visualization_msgs::msg::Marker>("/visualization_marker", 10);

    timer_ = this->create_wall_timer(std::chrono::milliseconds(50), std::bind(&Movement::control_loop, this));
    visual_timer = this->create_wall_timer(std::chrono::milliseconds(200),std::bind(&Movement::showVisualLine, this));

    // target_distance = 1.0;
    start_received = false;
    target_received = false;
    current_state = WAITING_FOR_TARGET;
    RCLCPP_INFO(this->get_logger(),"waiting for target");
  }

private:

  enum State{
    WAITING_FOR_TARGET,
    MOVING_TO_TARGET,
    TARGET_REACHED
  };

  double startX, startY;
  double targetX, targetY;
  double currentX, currentY;
  bool start_received;
  bool target_received;
  State current_state;
  std::vector<geometry_msgs::msg::Point> path_points;

  void odom_callback(const nav_msgs::msg::Odometry::SharedPtr msg)
  {
    currentX = msg->pose.pose.position.x;
    currentY = msg->pose.pose.position.y;

    if (!start_received)
    {
      startX = currentX;
      startY = currentY;
      start_received = true;   
      
      geometry_msgs::msg::Point start_point;
      start_point.x = startX;
      start_point.y = startY;
      start_point.z = 0.0;
      path_points.push_back(start_point);

    }
    
    if(current_state == MOVING_TO_TARGET && path_points.size() > 0)
    {

      const geometry_msgs::msg::Point& last_point = path_points.back();
      double dx = currentX - last_point.x;
      double dy = currentY - last_point.y;
      double distance = std::sqrt(dx * dx + dy * dy);

      if (distance> 0.03)  // 3cm threshold - INCREASED to reduce points
      {
        geometry_msgs::msg::Point new_point;
        new_point.x = currentX;
        new_point.y = currentY;
        new_point.z = 0.0;
        path_points.push_back(new_point);
      }
    }

  }

  void pose_callback(const geometry_msgs::msg::Point::SharedPtr msg)
  {
    if(current_state == WAITING_FOR_TARGET || current_state == TARGET_REACHED)
    {
      targetX = msg->x;
      targetY = msg->y;
      target_received = true;
      current_state = MOVING_TO_TARGET;

      // path_points.clear();
      geometry_msgs::msg::Point start_point;
      start_point.x = currentX;
      start_point.y = currentY;
      start_point.z = 0.0;
      path_points.push_back(start_point);


      RCLCPP_INFO(this->get_logger(), "New target received: (%.2f, %.2f)", targetX, targetY);
      RCLCPP_INFO(this->get_logger(), "Current position: (%.2f, %.2f)", currentX, currentY);
    }
  }

  void showVisualLine(){

    if(path_points.size() < 2){
      return;
    }

    auto marker = std::make_shared<visualization_msgs::msg::Marker>();

    marker->header.frame_id = "odom";
    marker->header.stamp = this->now();
    marker->ns = "movement_path";
    marker->id = 0;
    marker->type = visualization_msgs::msg::Marker::LINE_STRIP;
    marker->action = visualization_msgs::msg::Marker::ADD;


    marker->pose.position.x = 0;
    marker->pose.position.y = 0;
    marker->pose.position.z = 0;
    marker->pose.orientation.w = 1.0;
    marker->pose.orientation.x = 0.0;
    marker->pose.orientation.y = 0.0;
    marker->pose.orientation.z = 0.0;

    marker->scale.x = 0.1;
    marker->color.r = 1.0;
    marker->color.g = 0.0;
    marker->color.b = 0.0;
    marker->color.a = 1.0;

    marker->points = path_points;

    marker->lifetime = rclcpp::Duration::from_seconds(2.0); 

    // geometry_msgs::msg::Point start_point;
    // start_point.x = startX;
    // start_point.y = startY;
    // start_point.z = 0.0;
    // marker->points.push_back(start_point);

    // geometry_msgs::msg::Point current_point;
    // current_point.x = currentX;
    // current_point.y = currentY;
    // current_point.z = 0.0;
    // marker->points.push_back(current_point);

    visual_pub->publish(*marker);

  }

  void control_loop()
  {
    if (!start_received || !target_received)
      return;


    geometry_msgs::msg::Twist cmd;

    switch(current_state)
    {
      case WAITING_FOR_TARGET:
      cmd.linear.x = 0.0;
      cmd.linear.y = 0.0;
      cmd.angular.z = 0.0;
      break;

      case MOVING_TO_TARGET:
      {
        double dx = targetX - currentX;
        double dy = targetY - currentY;

        double distance_traveled = std::sqrt(dx * dx + dy * dy);

        // double error = target_distance - distance_traveled;


        if (distance_traveled > 0.01)
        {
          double angle = std::atan2(dy, dx);
          cmd.linear.x = 0.2 * std::cos(angle);
          cmd.linear.y = 0.2 * std::sin(angle);
          cmd.angular.z = 0.0;
          RCLCPP_INFO(this->get_logger(), "move robot");

        }
        else
        {
          cmd.linear.x = 0.0;
          cmd.linear.y = 0.0;
          cmd.angular.z = 0.0;

          current_state = TARGET_REACHED;
          target_received = false;

          std_msgs::msg::Bool reached_msg;
          reached_msg.data = true;
          reached_pub->publish(reached_msg);

          RCLCPP_INFO(this->get_logger(), "target reached");
          RCLCPP_INFO(this->get_logger(), "wait for next target");
        }
        break;
      }

      case TARGET_REACHED:
        cmd.linear.x = 0.0;
        cmd.linear.y = 0.0;
        cmd.angular.z = 0.0;
        break;
    }
    cmd_pub->publish(cmd);
  }  
  rclcpp::Publisher<geometry_msgs::msg::Twist>::SharedPtr cmd_pub;
  rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr visual_pub; 
  rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr odom_sub;
  rclcpp::Subscription<geometry_msgs::msg::Point>::SharedPtr pose_sub;
  rclcpp::Publisher<std_msgs::msg::Bool>::SharedPtr reached_pub;
  rclcpp::TimerBase::SharedPtr timer_;
  rclcpp::TimerBase::SharedPtr visual_timer;
};

int main(int argc, char**argv)
{
  rclcpp::init(argc, argv);
  rclcpp::spin(std::make_shared<Movement>());
  rclcpp::shutdown();
  return 0;
}