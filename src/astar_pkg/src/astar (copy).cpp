...
private:
  ...
  void wp_reached_callback(const std_msgs::msg::Bool::SharedPtr msg)
  {
    ...
    if (current_wp_ >= path_.size())
    {
      ...
      RCLCPP_INFO(this->get_logger(), "TARGET SELESAI - TUNGGU CLIMB");
    ...
    RCLCPP_INFO(this->get_logger(), "WP reached → (%d, %d)", path_[current_wp_].x,
                path_[current_wp_].y);
  ...
  void climbdone_callback(const std_msgs::msg::Bool::SharedPtr msg)
  {
    ...
    if (!path_.empty() && current_wp_ > 0 && current_wp_ <= path_.size())
    {
      auto wp = path_[current_wp_ - 1];
      robot_x = wp.x;
      robot_y = wp.y;
    ...
    if (wait_climb)
    {
      ...
      RCLCPP_INFO(this->get_logger(), "CLIMB DONE → NEXT TARGET");

      start_next_target(); // ✅ PINDAH KE SINI
      return;
    }
  ...
  void index_to_grid(int index, int &gx, int &gy)
  {
    ...
    // gx = grid_cols - 1 - (index % grid_cols);
    // gy = grid_rows - 1 - (index / grid_cols);

    // int x = index % grid_cols;
    // int y = index / grid_cols;

    // flip 180° (horizontal + vertical)
    // gx = (grid_cols - 1) - x;
    // gy = (grid_rows - 1) - y;
  ...
  void generate_path(int goal_x, int goal_y)
  {
    ...
    // int start_x = std::max(0, std::min(grid_cols - 1, start_x));
    // int start_y = std::max(0, std::min(grid_rows - 1, start_y));

    // int start_x = std::clamp(int(robot_x + 0.5), 0, grid_cols - 1);
    // int start_y = std::clamp(int(robot_y + 0.5), 0, grid_rows - 1);

    // robot_theta = yaw_to_dir(get_yaw(last_odom_msg));
